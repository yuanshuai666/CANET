...
data = np.load('/data6/shuaiYuan/code/Keras_bearing_fault_diagnosis-master/CICDDos2019/data/data.npy')
label = np.load('/data6/shuaiYuan/code/Keras_bearing_fault_diagnosis-master/CICDDos2019/data/label_new.npy', allow_pickle=True)
...
for train_index, test_index in kfold.split(x, y):
    ...
    acc.append(score)
    target_names = ['normals', 'DrDoS_DNS', 'DrDoS_LDAP', 'DrDoS_MSSQL', 'DrDoS_NetBIOS', 'DrDoS_NTP', 'DrDoS_SNMP', 'DrDoS_SSDP',
                    'DrDoS_UDP', 'Syn', 'TFTP', 'LDAP', 'NetBIOS',
                    'MSSQL', 'Portmap', 'UDP', 'UDP_lag']
    print(classification_report(y_eval, pred, target_names=target_names))
    cm = metrics.confusion_matrix(y_eval, pred)
    print(cm)
    tp = cm[1][1]+cm[2][2]+cm[3][3]+cm[4][4]+cm[5][5]+cm[6][6]+cm[7][7]+cm[8][8]+cm[9][9]+cm[10][10]+cm[11][11]+cm[12][12]+cm[13][13]+cm[14][14]+cm[15][15]+cm[16][16]
    fn = cm[1][0]+cm[2][0]+cm[3][0]+cm[4][0]+cm[5][0]+cm[6][0]+cm[7][0]+cm[8][0]+cm[9][0]+cm[10][0]+cm[11][0]+cm[12][0]+cm[13][0]+cm[14][0]+cm[15][0]+cm[16][0]
    fp = cm[0][1]+cm[0][2]+cm[0][3]+cm[0][4]+cm[0][5]+cm[0][6]+cm[0][7]+cm[0][8]+cm[0][9]+cm[0][10]+cm[0][11]+cm[0][12]+cm[0][13]+cm[0][14]+cm[0][15]+cm[0][16]
    tn = cm[0][0]
    DR = tp/(tp+fn)
    FPR = fp/(fp+tn)
